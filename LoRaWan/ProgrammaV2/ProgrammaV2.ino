...
int ambientLightDark = 750; //When baken needs to turn on
...
void setup() {
  ...
  //Inputs LDR's
  pinMode(A0, INPUT); //Ambient Light
...
void changeLightState(bool state) { //Function to turn on leds
  ...
  sendLoRaMessage("1:" + String(lights) + String(int(ina219.getBusVoltage_V() * 1000)) + ":" + String(statusCode));
}

void checkLightsLDR(unsigned int arr[]) {  //Get LDR value of lights and place in array
  arr[0] = GetLDRValue(A1);
  arr[1] = GetLDRValue(A2);
  arr[2] = GetLDRValue(A3);
}
...
void verifyLightStatus(unsigned int a[], unsigned int b[], unsigned int c[], unsigned int d[]) { //To check if the light value changed
  if ((a[0] + 20 < b[0]) && (c[0] < d[0])) { //Check if light is brighter after turn on and there is current flowing
    lights[0] = '1';
  } else {
    lights[0] = '0';
  }
  if ((a[1] + 20 < b[1]) && (c[1] < d[1])) {
    lights[2] = '1';
  } else {
    lights[2] = '0';
  }
  if ((a[2] + 20 < b[2]) && (c[2] < d[2])) {
    lights[3] = '1';
  } else {
    lights[3] = '0';
  }

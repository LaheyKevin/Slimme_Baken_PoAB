...
void setup() {
  ...
  if (!modem.begin(EU868)) {
    digitalWrite(6, HIGH);  //Turn led on when not started
    while (1) {}
  };
  ...
  if (!connected) {
    digitalWrite(6, HIGH);  //Turn led on when not connected
    while (1) {}
  }
...
void loop() {
  ...
  if (AmbientLight < ambientLightDark && !lightsStatus && automaticMode) { //If darker then value and leds are off & automatic
    changeLightState(true);
  } else if (AmbientLight > ambientLightDark && lightsStatus && automaticMode) {// If brighter then value and leds are on & automatic
